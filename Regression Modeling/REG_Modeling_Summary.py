...
'''
    RMSE: (Root Mean Squard Error) of test-set. (lower is better)
    ...
    mean(Residuals): avereage of Residuals of test-set. (close to 0 is better)
...
'''
    1. Gernerally, XGBoost has the best regression performace on metrics
        of {RMSE, MAE, R^2, EV}, Neural Network and Ranom Forest rank
        2nd and 3rd in these metrics.
        
    2. Gernerally, Polymial Regression, Decision Tree have better performance
    ...
    4. Gernerally, Linear Regression model like LASSO and Ridge Regression
        did not perform well (based on the metrics of  {RMSE, MAE, R^2, EV})
        on this case, but they are not the worst model.
        
    5. Gernerally, results with orginal features perform better than results
...
# -----------------------------------------------------------------------------
# Determine classification labing from results of regression models
'''
The classification lables of this case is "the last N cycles of
Remaining Useful Life" of each engines. So, the question is:
    
    "How should we determine N for classification labing"
    
We have got RMSE and MAE for regression models, all of these numbers could be
consider as "N" in for classification labing. (where the results with higher
R^2 and EV could be better).

Gernerally, we hope that N could be as small as it could be.
...
Though the RMSE and MAE are the most small one. But We could see that
most prediction from XGBoost regression are greater than true RUL,
which also resulted positive numbers of residuals. 

It mean that the results of XGBoost regression may be too optimistic for
our goal. So, let's abandon the results of XGBoost regression.

Based on the above perspective, the results of Neural Network and Ranom Forest
may be much worthy to consider where most of thier residuals are netive but 
much close to 0.

The results' numbers of RMSE and MAE of Neural Network and Ranom Forest 
...
# Summary
'''
We successsfully builded some regression models to predict Remaining Useful Life.
And we would use these results to determine N as 30 for classification labing
for next phrase.
'''

# save the dataframe of results comparision as "ref_reg_metrics" into CSV file
ref_reg_metrics = ref_reg_metrics.to_csv(r'C:\Users\Desktop\ref_reg_metrics.csv',\
